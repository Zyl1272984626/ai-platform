...
任务启动后可点击“启动自动研发”。平台会把任务契约编译为动态 Graph，通过项目内固定版本的 Codex app-server 启动主控线程，为线程设置 `[自动研发] 任务标题` 的用户可见名称并默认置顶，然后注入 `ai-platform-graph` MCP。该线程会持久化到 Codex 对应项目的侧边栏。Codex 领取可运行节点、按需创建子智能体并回写技术证据；平台根据依赖和门禁自动继续、返工或请求确认。人工验收不能由执行 Agent 代替。
