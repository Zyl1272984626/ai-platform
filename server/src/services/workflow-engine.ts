...
function loadRunState(runId: string): WorkflowRun | undefined {
...
export function startWorkflow(
  ...
  if (!definition) throw new Error(`Workflow not found: ${workflowName}`);
 ...
 * 从断点恢复
 */
export function resumeWorkflow(runId: string, emitter: EventEmitter): WorkflowRun {
  ...
  if (!definition) throw new Error(`Workflow definition not found: ${run.workflowName}`);
...
export function confirmStep(runId: string): void {
  ...
  if (stepRun) stepRun.status = 'success';
  run.status = 'running';
  saveRunState(run);
}
...
async function executeParallelSteps(
  ...
  const promises = steps.map(async (step) => {
    ...
    if (stepRun.status === 'success') return;
...
function groupSteps(steps: WorkflowStep[]): StepGroup[] {
  ...
  const parallelBuffer: WorkflowStep[] = [];
...
function evaluateCondition(condition: string, context: Record<string, unknown>): boolean {
  ...
  try {
    ...
    if (resolved.includes('==')) return new Function(`return ${resolved}`)() as boolean;
    if (resolved.includes('!=')) return new Function(`return ${resolved}`)() as boolean;
    // 简单布尔
...
function buildStepPrompt(step: WorkflowStep, input: Record<string, unknown>): string {
  return [
...
function parseStepOutput(output: string): Record<string, unknown> {
  ...
  if (resultMatch) {
    try {
      ...
      if (parsed.status === 'failed') throw new Error(parsed.error || 'Step failed');
    } catch (e) {
...
function addLog(run: WorkflowRun, level: string, message: string): void {
