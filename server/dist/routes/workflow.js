...
import { loadWorkflowDefinitions, startWorkflow, getWorkflowRun, listWorkflowRuns, resumeWorkflow, confirmStep, abortWorkflow, } from '../services/workflow-engine.js';
...
// 触发执行
workflowRouter.post('/:name/run', (req, res) => {
...
// 运行历史
workflowRouter.get('/runs', (_req, res) => {
    res.json(listWorkflowRuns());
});
// 单次运行详情
workflowRouter.get('/runs/:runId', (req, res) => {
...
// 断点恢复
workflowRouter.post('/runs/:runId/resume', (req, res) => {
    try {
        const emitter = new EventEmitter();
        res.writeHead(200, {
            'Content-Type': 'text/event-stream',
...
// 确认继续
workflowRouter.post('/runs/:runId/confirm', (req, res) => {
    try {
...
// 中止
workflowRouter.post('/runs/:runId/abort', (req, res) => {
