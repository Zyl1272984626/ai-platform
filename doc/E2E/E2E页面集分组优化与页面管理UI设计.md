...
1. 子应用去重
   - 如果两个子应用的路由列表完全相同（path 集合一致），合并为一个
   - 取第一个出现的子应用名作为代表
...
5. 小组自动合并
   - 如果某个分组只有 1 个页面，且子应用内存在多个单页分组：
     将所有单页分组合并为"其他"分组
   - 阈值可配置，默认 ≤1 页

6. 大组自动拆分
   - 如果某个分组超过 10 个页面，按第二段路径尝试细分
   - 细分后仍超 10 页的保留原样
```
...
| 子应用 | 页面集 | 说明 |
|--------|--------|------|
| index/chat（合并） | 2 集：用户门户(5页)、其他(3页) | depth=1 的合并 |
...
PUT    /api/projects/:id/pages         → 批量更新页面集（已有）
POST   /api/projects/:id/page-sets     → 新建页面集
PUT    /api/projects/:id/page-sets/:setId → 更新页面集（重命名等）
DELETE /api/projects/:id/page-sets/:setId → 删除页面集
POST   /api/projects/:id/page-sets/:setId/pages → 添加页面
...
| 文件 | 改动 | 说明 |
|------|------|------|
| `server/src/services/page-discovery.ts` | ~80 行 | `groupRoutes` 改用通用深度聚合算法 + 子应用去重 |
| `server/src/routes/projects.ts` | ~60 行 | 新增页面集/页面 CRUD API |
| `server/src/services/config.ts` | ~20 行 | 新增页面集/页面级别操作函数 |
| `web/src/views/SettingsView.vue` | ~200 行 | 页面管理弹窗 UI |
...
| 顺序 | 内容 | 说明 |
|------|------|------|
| 1 | 优化分组策略 | 修改 groupRoutes，用规则表替代简单前缀匹配 |
| 2 | 子应用去重 | index/chat 相同路由合并 |
| 3 | 页面管理 API | 后端 CRUD |
| 4 | 页面管理 UI | 前端弹窗 |
