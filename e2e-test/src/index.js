import { config, storagePath, generateRunId } from './config.js'
...
// admin 全部页面
const ADMIN_ALL_PAGES = [
  ...ADMIN_SYS_PAGES,
  ...ADMIN_PERMISSION_PAGES,
  ...ADMIN_BASE_PAGES,
  ...ADMIN_TASK_PAGES,
  ...ADMIN_PAGE_FACTORY_PAGES
]

// index 应用页面（需要 appId 参数）
...
// chat 对话页面 (需要 agentId 参数)
const CHAT_AGENT_ID = '40286e819e3f4ddf019e3f5adc98002d' // AI辅导员主智能体
const CHAT_PAGES = [
  { id: 'chat-home', name: '对话首页', url: `${BASE}/chat/index.html#/chat`, description: '聊天主页（无agentId）' },
...
// setting-app 应用设置页面（需要 appId 参数）(13页)
...
// 为每个 app 生成 index 页面
const INDEX_ALL_PAGES = [
  ...INDEX_BASE_PAGES,
  ...config.target.apps.flatMap(app => INDEX_APP_PAGES(app.appId))
]

// setting-system 全部页面
const SETTING_SYS_ALL_PAGES = [...SETTING_SYS_PAGES]

// setting-app 全部页面
const SETTING_APP_ALL_PAGES = [
  ...config.target.apps.flatMap(app => SETTING_APP_PAGES(app.appId))
]

// ===== 解析命令行参数 =====
const args = process.argv.slice(2)
const mode = getArgValue(args, '--mode', config.defaults.mode)
const scope = getArgValue(args, '--scope', 'admin-sys') // admin-sys | admin-all | index-all | setting-sys | setting-app | all
...
console.log('╔══════════════════════════════════════════════╗')
console.log('║     E2E 页面测试 v3.0 - AI Agent 测试        ║')
console.log('╠══════════════════════════════════════════════╣')
...
let pages = []
if (scope === 'admin-sys') {
  pages = ADMIN_SYS_PAGES
} else if (scope === 'admin-all') {
  pages = ADMIN_ALL_PAGES
} else if (scope === 'index-all') {
  pages = INDEX_ALL_PAGES
} else if (scope === 'setting-sys') {
  pages = SETTING_SYS_ALL_PAGES
} else if (scope === 'setting-app') {
  pages = SETTING_APP_ALL_PAGES
} else if (scope === 'all') {
  pages = [...ADMIN_ALL_PAGES, ...INDEX_ALL_PAGES, ...SETTING_SYS_ALL_PAGES, ...SETTING_APP_ALL_PAGES, ...CHAT_PAGES]
} else if (scope === 'chat') {
  pages = CHAT_PAGES
}
