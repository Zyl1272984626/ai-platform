 ...
 * E2E 测试报告生成器
 *
 * 供 Claude Code Skill 调用：传入测试结果 JSON，生成自包含 HTML 报告
 *
 * 用法：
 *   node src/report-generator.js <结果JSON文件路径>
 *   或
 *   echo '{"results":[...]}' | node src/report-generator.js
 */
...
if (process.argv[2]) {
  ...
  resultsData = JSON.parse(readFileSync(inputPath, 'utf-8'))
} else {
  // 从 stdin 读取（管道方式）
  let input = ''
  const chunks = []
  ...
  process.stdin.on('end', () => {
    resultsData = JSON.parse(Buffer.concat(chunks).toString())
    generateAndSave(resultsData)
  })
  process.stdin.on('error', () => {
    console.error('用法: node src/report-generator.js <结果JSON文件路径>')
    process.exit(1)
  })
}
...
function generateAndSave(data) {
  const { runId, mode, scope, results } = data
  ...
  const failed = results.filter(r => ['fail', 'error'].includes(r.overallStatus)).length
  const passRate = total > 0 ? Math.round((passed / total) * 100) : 0
  const avgScore = total > 0 ? Math.round(results.reduce((s, r) => s + r.score, 0) / total) : 0
  const allIssues = results.flatMap(r => r.issues.map(i => ({ ...i, pageName: r.pageName })))
  const totalDuration = results.reduce((s, r) => s + (r.duration || 0), 0)
  const totalAgentRounds = results.reduce((s, r) => s + (r.agentTimeline?.length || 0), 0)

  const reportId = runId || new Date().toISOString().replace(/[:.]/g, '-').substring(0, 19)
...
<title>E2E 页面测试报告 - ${reportId}</title>
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .issue-card .desc { font-size: 13px; color: #666; line-height: 1.6; }
  ...
  .timeline { background: #f9f9fb; border-radius: 8px; padding: 12px 16px; font-size: 13px; margin-bottom: 12px; }
  .timeline-round { padding: 6px 0; border-bottom: 1px solid #eee; }
  .timeline-round:last-child { border-bottom: none; }
...
<div class="header">
  <h1>E2E 页面测试报告 <span class="engine-tag">Claude Code AI Agent</span></h1>
  <div class="meta">${reportId} | 测试模式: ${mode || 'standard'} | 测试范围: ${scope || `${total}个页面`}</div>
</div>
...
<div class="filter-bar">
  ...
  <button class="filter-btn" onclick="filterPages('warning')">警告 (${warning})</button>
  ...
  <div class="content">
    ${results.map((r, i) => `
      <div class="page-detail" id="page-${i}" style="display: ${i === 0 ? 'block' : 'none'};" data-status="${r.overallStatus}">
        ...
        <div style="font-size:13px; color:#999; margin-bottom:12px;">耗时: ${r.duration ? (r.duration / 1000).toFixed(1) + '秒' : '未知'} | 模式: ${r.mode || mode}</div>

        ${r.agentTimeline?.length > 0 ? `
          <h4 style="margin-bottom:8px; color:#667eea;">AI Agent 测试时间线 (${r.agentTimeline.length}轮)</h4>
          <div class="timeline">
            ${r.agentTimeline.map((t, ti) => `
              <div class="timeline-round">
                <strong style="color:#667eea;">第${t.round}轮</strong>
                <span class="badge badge-${t.status === 'pass' ? 'pass' : t.status === 'error' ? 'fail' : 'warning'}">${t.status}</span>
                ...
                ${t.issues?.length > 0 ? `<div style="color:#ff4d4f; margin-top:4px; padding-left:24px;">${t.issues.map(ii => `⚠️ ${ii.title}`).join(' | ')}</div>` : ''}
              </div>
            `).join('')}
          </div>
        ` : ''}

        <h4 style="margin-bottom:8px;">检查项 (${r.checks?.length || 0}项)</h4>
        <ul class="check-list">
          ${(r.checks || []).map(c => `
            <li class="check-item">
        ...
        ${r.screenshots?.length > 0 ? `
          <h4 style="margin-top:16px; margin-bottom:8px;">截图证据 (${Array.isArray(r.screenshots) ? r.screenshots.length : 0}张)</h4>
          <div class="screenshot-grid">
            ${(Array.isArray(r.screenshots) ? r.screenshots : []).map(ss => {
              const path = typeof ss === 'string' ? ss : ss.file
              const name = typeof ss === 'string' ? ss.split('/').pop() : ss.name
              try {
                const imgData = readFileSync(path, 'base64')
                return `<div class="screenshot-thumb" onclick="showLightbox(this)"><img src="data:image/png;base64,${imgData}" alt="${name}" loading="lazy"><div class="name">${name}</div></div>`
              } catch (e) {
                return `<div class="screenshot-thumb"><div class="name">截图: ${name}（路径: ${path}）</div></div>`
              }
            }).join('')}
          </div>
        ` : ''}
...
function showLightbox(thumb) {
  ...
  // 保存报告
  const reportsDir = join(storagePath, 'reports')
  ...
  // 保存 JSON 数据
  const runsDir = join(storagePath, 'runs', reportId)
  if (!existsSync(runsDir)) mkdirSync(runsDir, { recursive: true })
  writeFileSync(join(runsDir, 'run.json'), JSON.stringify({ id: reportId, mode, scope, total, passed, warning, failed, avgScore, totalIssues: allIssues.length, reportPath }, null, 2), 'utf-8')

  return reportPath
}

function statusIcon(status) {
  return { pass: '✅', warning: '⚠️', fail: '❌', error: '💥' }[status] || '⏳'
}
